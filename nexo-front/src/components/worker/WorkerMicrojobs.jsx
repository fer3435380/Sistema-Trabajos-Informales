...
function WorkerMicrojobs({
  ...
  return (
    <div className="space-y-4">
      <section className="rounded-[1.45rem] border border-[var(--color-primary-border)] bg-white p-4 shadow-[0_18px_32px_rgba(4,44,83,0.05)] md:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-2">
            <p className="text-sm font-semibold text-[var(--color-primary)]">Oportunidades</p>
      ...
      {unlockedMicrojobs.length ? (
        <div className="grid grid-cols-1 gap-3 xl:grid-cols-2">
          {unlockedMicrojobs.map((microjob) => (
            <MicrojobCard
              key={microjob.id}
              microjob={microjob}
              isApplied={appliedMicrojobIds.has(microjob.id)}
              onOpenDetail={onOpenDetail}
              onApply={onApply}
              onOpenRequiredCourse={onOpenRequiredCourse}
            />
          ))}
        </div>
      ) : (
        <EmptyState
          title="No hay microtrabajos disponibles"
          description="Ajusta los filtros o completa cursos para desbloquear más oportunidades."
        />
      )}

      {lockedMicrojobs.length ? (
        <section className="space-y-3">
          <div className="space-y-2">
            <p className="text-sm font-semibold text-[var(--color-primary)]">Para desbloquear</p>
            <h3 className="text-xl font-bold tracking-[-0.04em] text-[var(--color-primary-strong)]">
              Oportunidades que requieren un curso
            </h3>
          </div>
          <div className="grid grid-cols-1 gap-3 xl:grid-cols-2">
            {lockedMicrojobs.map((microjob) => (
              <MicrojobCard
                key={microjob.id}
                microjob={microjob}
                isApplied={false}
...
export default WorkerMicrojobs
